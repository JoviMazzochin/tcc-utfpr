...
public class JuntaResultsGradienteGray {
    public static void main(String[] args) throws Exception {
        String path = "/Users/joaomazzochin/Documents/University/UTFPR 5º SEMESTRE/TCC1 /Database/GradienteGray/20train10test/fake";
        File folder = new File(path); //fake
        File[] files = folder.listFiles();
        final int TAM = 256;

        File juntos = new File(path + "/../onlyGray/");
...
//            System.out.println(files[k].getAbsolutePath());
            ...
            img.erode(Morphology.STRUCT_SUP,2);
            img.dilate(Morphology.STRUCT_SUP,2);
            img.resize(TAM, TAM);
            img2.resize(TAM, TAM);
            ...
            for (int i=0; i<TAM; i++){
                for (int j=0; j<TAM; j++){
                    for (int b=0; b<3; b++) {
                        imgOut.setPixel(j, i, b, img2.getPixel(j, i));
